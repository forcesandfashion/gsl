...
export default function SubAdminRanges() {
  ...
  const handleAcceptRange = async (rangeId) => {
    try {
      ...
      setRanges(prev => prev.map(range => 
        range.id === rangeId 
          ? { ...range, status: 'active', updatedAt: new Date() }
          : range
      ));
  ...
  // Block active range (change status to blocked)
  const handleBlockRange = async (rangeId) => {
    try {
      ...
      setRanges(prev => prev.map(range => 
        range.id === rangeId 
          ? { ...range, status: 'blocked', updatedAt: new Date() }
          : range
      ));
  ...
  // Unblock blocked range (change status to active)
  ...
  // Delete range (used for rejecting pending ranges or deleting any range)
  const handleDeleteRange = async () => {
    ...
    try {
      ...
      closeModal();
      alert(`Range ${activeTab === 'pending' ? 'rejected' : 'deleted'} successfully!`);
    } catch (error) {
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-xl max-w-md w-full p-6 shadow-2xl">
            ...
            <div className="bg-gray-50 rounded-lg p-4 mb-4">
              <p className="text-sm text-gray-700 font-medium">{selectedRange.name}</p>
              <p className="text-xs text-gray-500">{selectedRange.ownerEmail}</p>
            </div>
