...
export default function RangeListingForm() {
  ...
  const createAuditLog = async (rangeId: string, rangeData: any) => {
    ...
    try {
      const auditLogData: AuditLog = {
        ...
        details: {
          ...
          pricePerHour: rangeData.pricePerHour,
          contactNumber: rangeData.contactNumber,
          facilities: rangeData.facilities,
          maxBookingsPerSlot: rangeData.maxBookingsPerSlot,
      ...
      // Add to range-owners/{ownerId}/actions collection
      await addDoc(collection(db, "range-owners", ownerId, "actions"), auditLogData);
      
      console.log("Audit log created successfully");
    } catch (error) {
      console.error("Failed to create audit log:", error);
      // Don't throw error as this shouldn't prevent range creation
    }
  };
  ...
  const handleRangeImagesChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Managers have access to unlimited images (premium features)
  ...
  const handleVideoChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0];
    if (file) {
      // Managers have access to premium video limits (500MB)
      const maxSize = 500 * 1024 * 1024; // 500MB for managers
      if (file.size > maxSize) {
        toast({
          title: "File Too Large",
          description: "Video size must be less than 500MB",
          variant: "destructive"
        });
        return;
      }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await createAuditLog(docRef.id, rangeData);
        ...
        <div className="text-center mb-8">
          ...
          <div className="mt-4 p-4 bg-gradient-to-r from-blue-50 to-purple-50 border border-blue-200 rounded-xl shadow-sm">
            ...
            <div className="text-sm text-blue-700 space-y-1">
              ...
              <p>• Owner logo integration</p>
              <p>• Complete audit trail logging</p>
            </div>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
          ...
          <div className="p-6 bg-gradient-to-r from-orange-50 to-red-50 rounded-2xl border border-orange-200">
            <Label htmlFor="rangeImages" className="flex items-center gap-2 font-semibold text-gray-700 mb-4">
              ...
              <span className="text-sm font-normal text-gray-500 bg-white px-2 py-1 rounded-full">
                {rangeImageFiles.length} images uploaded
              </span>
            </Label>
            <Input
              id="rangeImages"
          ...
          <div className="p-6 bg-gradient-to-r from-purple-50 to-pink-50 rounded-2xl border border-purple-200">
            <Label className="flex items-center gap-2 text-lg font-semibold text-gray-700 mb-6">
              ...
              <span className="text-sm font-normal text-gray-500">(Both options available)</span>
            </Label>
            ...
            <div className="mt-4 p-3 bg-blue-50 border border-blue-200 rounded-lg">
              <p className="text-sm text-blue-700">
                <strong>Manager Access:</strong> You can use both YouTube URL and video upload simultaneously. 
                Video uploads support up to 500MB file size.
              </p>
            </div>
          </div>
