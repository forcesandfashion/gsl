...
const SubAdminEvent: React.FC = () => {
  ...
  const updateEventStatus = async (eventId: string, newStatus: EventStatus): Promise<void> => {
    try {
      ...
      alert(`Event ${newStatus === 'active' ? 'approved' : newStatus === 'blocked' ? 'blocked' : 'updated'} successfully!`);
    } catch (err) {
  ...
  // Reject/Delete pending event only
  const rejectPendingEvent = async (eventId: string): Promise<void> => {
    if (!window.confirm('Are you sure you want to reject this pending event? This will permanently delete it.')) {
      return;
    }
    ...
    try {
      ...
      alert('Pending event rejected and deleted successfully!');
    } catch (err) {
      console.error('Error rejecting pending event:', err);
      alert('Failed to reject pending event. Please try again.');
    } finally {
      setActionLoading(prev => ({ ...prev, [eventId]: false }));
    }
  };
          ...
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div>
              <h1 className="text-3xl font-bold text-gray-800 flex items-center gap-3">
                ...
                Sub Admin Events Management
              </h1>
              <p className="text-gray-600 mt-2">Review and manage shooting events</p>
            </div>
                          ...
                          <div className="flex flex-wrap gap-2">
                            ...
                            {activeTab === 'active' && (
                              <button
                                ...
                                disabled={isLoading}
                                className="px-3 py-2 bg-yellow-600 text-white rounded-lg hover:bg-yellow-700 transition-colors flex items-center gap-2 text-sm disabled:opacity-50 disabled:cursor-not-allowed"
                              >
                                {isLoading ? (
                                  <div className="h-3 w-3 animate-spin border-2 border-white border-t-transparent rounded-full"></div>
                                ) : (
